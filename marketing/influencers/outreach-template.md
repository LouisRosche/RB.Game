# Influencer Outreach — Alchemy Academy

## Strategy

Don't chase big Roblox YouTubers (10M+ subs). They're expensive, unreliable,
and their audiences are broad. Instead target:

1. **Science/education creators** (50K-500K) who would genuinely love the concept
2. **Indie game discovery** channels (10K-200K) that review small games
3. **Roblox mid-tier** creators (5K-100K) with engaged audiences
4. **Speedrunning/challenge** creators (any size) — Legendary drop rate challenges are tailor-made for them

ROI principle from research: one authentic creator post from a 50K science
channel outperforms a paid post from a 1M Roblox channel.

---

## Outreach Tiers

### Tier 1 — Free Keys / Press (Phase 3)
Target: Science educators, indie game reviewers
Ask: "Try it free, share if you enjoy it" — no payment, no obligation

### Tier 2 — Revenue Share (Phase 3, after proof of income)
Target: Roblox content creators
Deal: 10-20% of DevEx revenue attributable to their referral code
(Roblox doesn't have built-in referral tracking — use a promo code in the game
that players can type for a starting bonus, tied to a creator's handle)

### Tier 3 — Paid Sponsors (Phase 3, only after $500/month sustained)
Maximum: $50-200 per sponsored post
Only if creator genuinely fits (science, education, or cozy games audience)

---

## Email Template — Science Educator

**Subject**: I made a Roblox game around [their topic] — thought you might like it

```
Hi [Name],

I've been watching your videos on [specific topic] for a while — the one about
[specific video title] is exactly why I built a feature into my new game.

Alchemy Academy is a free Roblox game where every potion you brew unlocks a
real science fact. Moonpetal potions teach bioluminescence, Fireroot potions
teach geothermal energy, and so on. The Alchemist's Journal has 10 science
notes written to match your audience's age range.

I made it because I wanted a game that treats curiosity as the reward, not
dopamine manipulation. No loot boxes, full probability transparency, no FOMO.

If you'd like to try it: [Roblox game link]

I'm not asking you to cover it — just thought you might find it interesting
given what you make. No obligation either way.

[Your name]
```

---

## Email Template — Indie Game Reviewer

**Subject**: Small Roblox idle game with transparent odds and real science

```
Hi [Name],

Quick pitch: Alchemy Academy is a free solo-dev Roblox idle game I just launched.

What's different from the genre:
- Every potion recipe unlocks a real science fact (bioluminescence, nuclear fusion, etc.)
- All mutation odds are shown upfront — no mystery mechanics
- No pay-to-win, no FOMO timers, no loot boxes

It runs on low-end hardware, session length ~10 minutes.

If it sounds like something your audience would enjoy: [link]
Happy to answer any questions.

[Your name]
```

---

## Tracking Sheet

| Creator | Platform | Subs/views | Date reached | Response | Outcome |
|---------|----------|-----------|--------------|----------|---------|
| | | | | | |

---

## Who to Contact (Research Starting Points)

**Science educators on YouTube/TikTok**:
- Search: "bioluminescence explained", "geothermal energy kids", "nuclear fusion simple"
- Filter: 10K-500K subscribers, last upload < 30 days, engagement rate > 3%

**Indie game discovery**:
- Search: "hidden gem Roblox games 2025", "best indie games under $10 Steam"
- Filter: covers small games, positive community tone

**Roblox community**:
- Roblox DevForum showcase section (other devs who might share)
- r/roblox (post a trailer, not a sales pitch)
- r/indiegaming (Steam version)

---

## What Not to Do

- Do not mass-email 100+ creators with the same template
- Do not offer payment before having sustained revenue (Phase 3 minimum)
- Do not contact creators whose audience is primarily under 10 years old for the
  Steam version (COPPA/GDPR compliance risk)
- Do not ask for coverage — offer access and let them decide
